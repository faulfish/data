 ...
 * ----------------
 *
 * $Log:   //BQT/VM67/Prjbase3/QSyncher_V2/Code/Components/UIFramework/LayoutManager/LayoutManager.cpp.-arc  $
...
namespace QSYNC_LIB
{
	CLayoutManager::CLayoutManager(CWnd* pThis, INotify* pINotify)
		:m_qXMLdoc(&m_qXMLlib)
		,m_strFileName(g_strDeafultPath + strQXML)
		,m_bLoad(false)
		,m_pWnd(pThis)
		,m_pINotify(pINotify)
	{ 
	...
	bool CLayoutManager::SaveToXML(const XMLElement& Element, const TSTRING& strFilePath)
	{
		...
		if(!strFilePath.empty())
		{
			...
			int nsize = m_EleRoot.getAllElement()->size();
