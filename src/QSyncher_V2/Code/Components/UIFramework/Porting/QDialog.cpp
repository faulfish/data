...
BOOL QSYNC_LIB::CQDialog::OnInitDialog()
{
...
BOOL QSYNC_LIB::CQDialog::SetWindowText(LPCTSTR lpszString)
{
	m_strWindowText = lpszString;
	return TRUE;
}
void QSYNC_LIB::CQDialog::GetWindowText(CString& strText)
{
	strText = m_strWindowText.c_str();
}

void QSYNC_LIB::CQDialog::SetWindowRegion(LPRECT& pRect, GeometryRect& rcClient) {
    RECT rcClient2 = {0};
...
std::wstring QSYNC_LIB::CQDialog::GetWString(const TSTRING& strID)
{
...
void QSYNC_LIB::CQDialog::SetDlgMoveable(bool bMoveable)
{
	m_DlgMoveable = bMoveable;
}

void QSYNC_LIB::CQDialog::OnLButtonDown( UINT nFlags, CPoint point)
{
	if(m_DlgMoveable)
		PostMessage( WM_NCLBUTTONDOWN, HTCAPTION, MAKELPARAM( point.x, point.y));
}

BOOL QSYNC_LIB::CQDialog::PreTranslateMessage(MSG* pMsg)
{
	switch(pMsg->message)
	{
	case WM_SYSKEYDOWN:
...
void QSYNC_LIB::CQDialog::Disconnect(const TSTRING& strTitle,const TSTRING& strContent,bool bShowMessage)
{
