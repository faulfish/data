...
namespace QSYNC_LIB
{
	...
	IMPLEMENT_DYNAMIC(CQPortingTreeUI, CTreeCtrl)
		...
		CQPortingTreeUI::CQPortingTreeUI():m_pImageList(NULL),m_pILocalization(NULL)
	{
	...
	bool CQPortingTreeUI::ExpandBranch(HTREEITEM hHeadTreeItem,bool bExpand)
	{
		...
		if( ItemHasChildren(hHeadTreeItem) )
		{
	...
	void CQPortingTreeUI::DrawString( CDC* pDC, bool bSelected, bool bFocus,const CRect& rcItem , const CString& strTemp)
	{
		...
		SolidBrush FocusBrush(Color(255,0,128,192));
		SolidBrush UnfocusBrush(Color(255,128,128,128));
		...
		if(pmyFont)
		{
			...
			INT linesFilled;
			graphics.MeasureString(strTemp,strTemp.GetLength(),pmyFont,layoutRect,&stringFormat,&rcBound,&codepointsFitted,&linesFilled);
			layoutRect.Width = rcBound.Width + 100;

			if(bSelected)
				if(bFocus)
					graphics.DrawString(strTemp,-1,pmyFont,layoutRect,&stringFormat,&FocusBrush);
				else
					graphics.DrawString(strTemp,-1,pmyFont,layoutRect,&stringFormat,&UnfocusBrush);
			else
				graphics.DrawString(strTemp,-1,pmyFont,layoutRect,&stringFormat,&blackBrush);
