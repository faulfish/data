...
// IDocHostUIHandler
	...
	STDMETHOD(GetOptionKeyPath)(__deref_out LPOLESTR* pchKey, DWORD dwReserved)
	{
		HRESULT hr = S_FALSE;
		if (pchKey == NULL)
        {
		...
		*pchKey = NULL;
		...
		else
		{
            if (m_bstrOptionKeyPath.Length() != 0)
			{
				...
				if(!ocscpy_s(pStr, (nByteLength/sizeof(OLECHAR)) + 1, m_bstrOptionKeyPath.m_str))
				{
					return E_FAIL;
				}
				*pchKey = pStr;
	...
	STDMETHOD(TranslateUrl)(DWORD dwTranslate, __in OLECHAR* pchURLIn, __deref_out OLECHAR** ppchURLOut)
	{
...
// IOleContainer
	STDMETHOD(ParseDisplayName)(IBindCtx* /*pbc*/, __in_opt LPOLESTR /*pszDisplayName*/, ULONG* /*pchEaten*/, IMoniker** /*ppmkOut*/)
	{
