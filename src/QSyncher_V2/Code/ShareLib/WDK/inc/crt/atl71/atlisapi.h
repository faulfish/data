		...
		while (szQueryString && *szQueryString)
		{
			...
			while (*szQueryString)
			{
				...
				else if (*szQueryString == '%')
				{
					...
					{
						ch1 = AtlHexValue(szQueryString[1]);
						ch2 = AtlHexValue(szQueryString[2]);
						if (ch1 < 0 || ch2 < 0)
						{
							return FALSE;
						}
						*szUrlCurrent = (CHAR)(16*ch1+ch2);
						szQueryString += 2;
					}
					else
						*szUrlCurrent = '\0';
...
public:
	...
	ATL_NOINLINE BOOL InitFromPost() throw()
	{
		_ATLTRY
		{
			...
			if ((dwBytesTotal+1) < dwBytesTotal )
			{
				return FALSE;
			}
...
public:
	...
	__success(return>=0) __checkReturn HRESULT STDMETHODCALLTYPE QueryInterface(__in REFIID riid, __deref_out void **ppv)
	{
		if (!ppv)
			return E_POINTER;
...
public:
	...
	virtual HTTP_CODE LoadDllHandler(__in LPCSTR szFileName, __in AtlServerRequest *pRequestInfo)
	{
		ATLASSERT(szFileName);
		ATLENSURE(pRequestInfo);
	...
	BOOL ProcessDebug(__inout AtlServerRequest *pRequestInfo)
{
	...
	__success(return>=0) __checkReturn HRESULT STDMETHODCALLTYPE QueryInterface(REFIID riid, __deref_out void **ppv)
	{
		if (!ppv)
			return E_POINTER;
	...
	__success(return>=0) __checkReturn HRESULT GetService(__in REFGUID guidService, __in REFIID riid, __deref_out void **ppvObject) throw()
	{
		if (!ppvObject)
			return E_POINTER;
