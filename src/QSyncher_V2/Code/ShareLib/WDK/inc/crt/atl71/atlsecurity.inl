...
inline bool CSid::LoadAccount(LPCTSTR pszAccountName, LPCTSTR pszSystem /* = NULL */) throw(...)
{
	...
	DWORD cchDomain = dwDomainSize;

	BOOL bSuccess = ::LookupAccountName(pszSystem, pszAccountName, pSid, &cbSid, szDomain, &cchDomain, &m_sidnameuse);
	if (!bSuccess && ::GetLastError() == ERROR_INSUFFICIENT_BUFFER)
	{
		...
		if (cchDomain > dwDomainSize)
		{
			// Failed because domain was not big enough, reallocate it.
			buffDomain.Reallocate(cchDomain * sizeof(TCHAR));
			szDomain = static_cast<TCHAR *>(buffDomain);
		}
		bSuccess = ::LookupAccountName(pszSystem, pszAccountName, pSid, &cbSid, szDomain, &cchDomain, &m_sidnameuse);
	}
	...
	if (bSuccess)
	{
		...
		// LookupAccountName doesn't change cbSid on success (although it changes cchDomain)
		m_bValid = true;
...
inline void CSid::GetAccountNameAndDomain() const throw(...)
{
	...
	if(::LookupAccountSid(m_strSystem, _GetPSID(), szName, &cchName, szDomain, &cchDomain, &m_sidnameuse))
	{
	...
	else
	{
		...
		{
			LPTSTR pszName = m_strAccountName.GetBuffer(cchName * sizeof(TCHAR));
			LPTSTR pszDomain = m_strDomain.GetBuffer(cchDomain * sizeof(TCHAR));

			if (!::LookupAccountSid(m_strSystem, _GetPSID(), pszName, &cchName, pszDomain, &cchDomain, &m_sidnameuse))
				AtlThrowLastWin32();
