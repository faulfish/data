                                        ...
                                        if (pInfo->m_pStrToolTip[i] == NULL)
					{
						ATLTRY(pInfo->m_pStrToolTip[i] = new OLECHAR[lstrlen(szStatusBar) + 1]);
                                        ...
                                        if (pInfo->m_pStrToolTip[i] == NULL)
                                                continue;
                                        USES_CONVERSION_EX;
					LPOLESTR lpSrc = T2OLE_EX(szStatusBar, _ATL_SAFE_ALLOCA_DEF_THRESHOLD);
					if(lpSrc == NULL || 0 != memcpy_s(pInfo->m_pStrToolTip[i], (lstrlen(szStatusBar) + 1) * sizeof(WCHAR), lpSrc, (ocslen(lpSrc)+1)*sizeof(WCHAR)))
					{
						DeleteObject(hBitmap);
						return E_FAIL;
					}
