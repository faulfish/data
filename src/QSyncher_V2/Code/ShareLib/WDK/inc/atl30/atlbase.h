...
ATLAPI AtlSetErrorInfo2(const CLSID& clsid, UINT nID, DWORD dwHelpID,
...
public:
...
#ifdef ATLBASE_SMARTPTR_EXTRA_CTORS
        // i'd like to just turn these on. but, the LPCOLESTR one creates an ambiguous overload wherever
        // there's code that doesn't understand the default ctor init to NULL and explicitly does so at the
        // point of declaration.  its probably too much work to find all these and fix them
        ...
        T** operator&()
        {
#ifdef ATLBASE_DISABLE_ADDRESSOF_OPERATOR_ASSERT
                // this is unavoidably triggered by legitimate access inside templates.  example:
                // using a CComPtr in an stl container and then using IEnumonSTLImpl the ::Next
        ...
        // if INTERFACE isn't a base then we'll get a compile error on the assignment
        template<class INTERFACE> HRESULT CopyTo(INTERFACE** ppT)
        {
#ifdef ATLBASE_DISABLE_COPYTO_ASSERT
...
// with stl 7 this class is no longer necessary, and you can convert to stl 7 without converting away from atl 3.0
// if you remove references to this class you save an extra addref/release on every assignment in or out
// of the stl container caused by the copy ctor of CAdapt.
...
// Operations
        ...
        BOOL Add(T& t)
        {
            const int nOldSize = m_nSize;
            const int nNewSize = (nOldSize + 1);
            const int nOldAllocSize = m_nAllocSize;
            if (nNewSize <= 0)
                return FALSE;
            if (nOldSize == nOldAllocSize)
            {
                T* aT;
                if (nOldSize > (INT_MAX / 2))
                    return FALSE;
                if (nOldSize > (SIZE_MAX / 2 / sizeof(T)))
                    return FALSE;
                const int nNewAllocSize = ((nOldAllocSize == 0) ? 1 : (nOldSize * 2));
                aT = (T*)realloc(m_aT, ((size_t)nNewAllocSize) * sizeof(T));
                if(aT == NULL)
                        return FALSE;
...
public:
...
#ifdef ATLBASE_SYMMETRIC_BSTR_OPS
        bool operator>(BSTR bstrSrc) const
        {
                if (bstrSrc == NULL && m_str == NULL)
                        return false;
...
#ifdef ATLBASE_SYMMETRIC_BSTR_OPS
        // i'd like to just add these extra operators, but since they're defined for BSTR & LPCTR
        // they have the potential to create ambiguous operator references in existing code
        // where it compares to NULL.  the existing op== already creates this ambiguity &&
        // its fixable by using ! instead but creates unnecessary churn.  
        bool operator!=(BSTR bstrSrc) const
        {
        ...
        HRESULT WriteToStream(IStream* pStream)
        {
                ...
                ULONG cb;
#ifdef ATLBASE_ACCURATE_BSTR_PERSISTENCE
                // SysStringByteLen returns the real length of the actually allocated buffer as 
                // specified at ((DWORD*)buffer)[-1].
                // there's no reason to write out the extra NULL that sysalloc hides there
                // unless you just like wasting space
                ULONG cbStrLen = m_str ? SysStringByteLen(m_str) : 0;
#else
                // however, there are components(CMI) that compute hashes on this value and this causes
                // mismatches with old stores.
        ...
        {
            pSize->QuadPart = sizeof(ULONG);
#ifdef ATLBASE_ACCURATE_BSTR_PERSISTENCE
            pSize->QuadPart += m_str ? SysStringByteLen(m_str) : 0;
#else
            pSize->QuadPart += m_str ? SysStringByteLen(m_str) + sizeof(OLECHAR) : 0;
#endif
            return NOERROR;
        }
        HRESULT ReadFromStream(IStream* pStream)
        {
                ...
                {
#ifdef ATLBASE_ACCURATE_BSTR_PERSISTENCE
                    // i fixed it so that no extra is written(see above).  
...
#ifdef ATLBASE_ACCURATE_BSTR_PERSISTENCE
                            ATLASSERT((reinterpret_cast<DWORD*>(m_str))[-1] == cbStrLen);
                            // make sure sysallocstring put the null back even though we didn't save it(see comment in write)
                            ATLASSERT((reinterpret_cast<OLECHAR*>(m_str))[cbStrLen / sizeof(OLECHAR)] == 0);
#endif
                            hr = pStream->Read(reinterpret_cast<void*>(m_str), cbStrLen, NULL);
                    }
                }
                if (hr == S_FALSE)
                        hr = E_FAIL;
...
public:
        ...
        HRESULT CopyTo(VARIANT* pDest) {
            bool fBlob = false;
            if (vt == VT_BSTR_BLOB) {
                vt = VT_BSTR;
                fBlob = true;
            }
            HRESULT hr = ::VariantCopy(const_cast<VARIANT*>(pDest), this); 
            if (fBlob) {
                vt = VT_BSTR_BLOB;
            }
            return hr;
...
inline HRESULT CComVariant::WriteToStream(IStream* pStream)
{
        ...
        case VT_ARRAY | VT_UI1: {
	        ...
	        __int64 len = 0;
            if (parray) {
		        hr = SafeArrayAccessData(parray, reinterpret_cast<LPVOID *>(&pData));
		        if (FAILED(hr)) {
			        return hr;
		        }
		        for (int i = parray->cDims; i--;) {
			        len += parray->rgsabound[i].cElements;
		        }
		        len *= parray->cbElements;
                hr = pStream->Write((void*) &len, sizeof(len), NULL);
                if (FAILED(hr)) {
                    return hr;
                }
                hr = pStream->Write((void*) pData, (ULONG) len, NULL);
                if (FAILED(hr)) {
                    return hr;
                }
			    SafeArrayUnaccessData(parray);
            } else {
                return pStream->Write((void*) &len, sizeof(len), NULL);
            }
            return NOERROR;
	    } break;
        case VT_UI1:
        ...
        if (vt != VT_BSTR  && vt != VT_BSTR_BLOB)
        {
                ...
                bstrWrite.Attach( bstrVal); // don't do unnecessary copy
                                            // it can be expensive and it doesn't work for VT_BSTR_BLOB 
                                            // w/ embedded nulls
        }

        hr = bstrWrite.WriteToStream(pStream);
        bstrWrite.Detach(); // don't double free, we didn't create a copy
        return hr;
}

inline HRESULT CComVariant::GetSizeMax(ULARGE_INTEGER *pSize)
{
    ...
    case VT_DISPATCH:
        {
            ...
            if (punkVal != NULL)
            {
                HRESULT hr = punkVal->QueryInterface(IID_IPersistStream, (void**)&spStream);
                if (FAILED(hr))
                    return hr;
            }
            if (spStream != NULL) {
                pSize->QuadPart += sizeof(CLSID);
                ULARGE_INTEGER s;
                HRESULT hr = spStream->GetSizeMax(&s);
                if (SUCCEEDED(hr)) {
    ...
    case VT_ARRAY | VT_UI1: {
		LPBYTE pData = NULL;
	    __int64 len = 0;
        pSize->QuadPart += sizeof(len);
        if (parray) {
		    HRESULT hr = SafeArrayAccessData(parray, reinterpret_cast<LPVOID *>(&pData));
		    if (FAILED(hr)) {
			    return hr;
		    }
		    for (int i = parray->cDims; i--;) {
    ...
    case VT_UI1:
    case VT_I1:
        pSize->QuadPart += sizeof(BYTE);
        return NOERROR;
    case VT_I2:
    case VT_UI2:
    case VT_BOOL:
        pSize->QuadPart += sizeof(short);
        return NOERROR;
    case VT_I4:
    case VT_UI4:
    case VT_R4:
    ...
    case VT_UINT:
    case VT_ERROR:
        pSize->QuadPart += sizeof(long);
        return NOERROR;
    case VT_UI8:
    case VT_I8:
        pSize->QuadPart += sizeof(__int64);
        break;
    case VT_R8:
    case VT_CY:
    case VT_DATE:
        pSize->QuadPart += sizeof(double);
        return NOERROR;
    default:
        break;
    }
    ...
    if (vt != VT_BSTR  && vt != VT_BSTR_BLOB)
    {
        ...
        bstrWrite.Attach( bstrVal); // don't do unnecessary copy
                                    // its expensive and it doesn't work for VT_BSTR_BLOB 
                                    // w/ embedded nulls
    }

    ULARGE_INTEGER s;
    HRESULT hr = bstrWrite.GetSizeMax(&s);
    bstrWrite.Detach(); // don't double free, we didn't create a copy
    if (SUCCEEDED(hr)) {
        pSize->QuadPart += s.QuadPart;
    }
...
inline HRESULT CComVariant::ReadFromStream(__inout IStream* pStream, VARTYPE vtExpected /* = VT_EMPTY */ )
{
        ...
        case VT_ARRAY | VT_UI1: {
			...
			rgsabound[0].cElements = (ULONG) cbSize;
        ...
        if (vtRead != VT_BSTR && vtRead != VT_BSTR_BLOB) {
...
// even though currently ulOptions are reserved and must be 0, by
// by allowing ulOptions to pass through it lets other interfaces completely reflect the entire
// RegOpenKeyEx signature so that if ulOptions are ever used for anything the other interfaces
...
inline LONG CRegKey::Create(__in HKEY hKeyParent, __in_z LPCTSTR lpszKeyName,
        ...
        __in_opt LPSECURITY_ATTRIBUTES lpSecAttr, __in_opt LPDWORD lpdwDisposition)
{
...
// allow ulOptions to pass through.  see comment above
...
inline LONG CRegKey::QueryStringValue(__in_z_opt LPCTSTR pszValueName, __out_ecount_part_z_opt(*pnChars, *pnChars) LPTSTR pszValue, __inout ULONG* pnChars)
{
	LONG lRes;
	DWORD dwType;
	ULONG nBytes;
...
inline LONG CRegKey::SetStringValue(__in_z_opt LPCTSTR pszValueName, __in_z LPCTSTR pszValue, __in DWORD dwType)
{
	ATLASSERT(m_hKey != NULL);
...
ATLINLINE ATLAPI AtlSetErrorInfo2(const CLSID& clsid, UINT nID, DWORD dwHelpID,
        LPCOLESTR lpszHelpFile, const IID& iid, HRESULT hRes, HINSTANCE hInst, va_list* Arguments)
{
        USES_CONVERSION_EX;
        TCHAR szDesc[1024];
        szDesc[0] = NULL;
        // mc compiler can easily generate nID > 0xffff thus lpstr version above can't work
        // plus need FormatMessage
        if (LoadString(hInst, nID, szDesc, 1024) == 0)
        {
            if(FormatMessage(FORMAT_MESSAGE_FROM_HMODULE, hInst, nID, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), szDesc, 1023, Arguments) == 0){
                ATLASSERT(FALSE);
#pragma warning(push)
#pragma warning(disable:4995) // lstrcpy is deprecated
                lstrcpy(szDesc, _T("Unknown Error"));
#pragma warning(pop)
        ...
        LPCOLESTR lpszDesc = T2OLE_EX(szDesc, _ATL_SAFE_ALLOCA_DEF_THRESHOLD);
#ifndef _UNICODE
        if(lpszDesc == NULL) 
                return E_OUTOFMEMORY;
#endif                        
        return AtlSetErrorInfo(clsid, lpszDesc, dwHelpID, lpszHelpFile, iid, hRes, hInst);
}
...
ATLINLINE ATLAPI AtlModuleRegisterTypeLib(_ATL_MODULE* pM, LPCOLESTR lpszIndex)
{
        ...
        if (SUCCEEDED(hr))
        {
                ...
                {
                        OLECHAR szDir[_MAX_PATH+1];

                        if(SysStringLen(bstrPath) > _MAX_PATH )
                                return CO_E_PATHTOOLONG;

                        ocscpy(szDir, bstrPath);
			UINT len = AtlGetDirLen(szDir);
			if (len < _MAX_PATH+1)
				szDir[len] = 0;
