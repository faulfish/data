	...
	ITypeInfo** pptinfo)
{
	...
	HRESULT hRes = GetTI(lcid, pptinfo);
	return hRes;
}
...
LONG CRegKey::QueryDWORDValue(LPCTSTR pszValueName, DWORD& dwValue)
{
	LONG lRes;
	ULONG nBytes;
	DWORD dwType;
...
LONG CRegKey::QueryStringValue(__in_z_opt LPCTSTR pszValueName, __out_ecount_part_z_opt(*pnChars, *pnChars) LPTSTR pszValue, __inout ULONG* pnChars)
{
	LONG lRes;
	DWORD dwType;
	ULONG nBytes;
...
LONG CRegKey::SetDWORDValue(LPCTSTR pszValueName, DWORD dwValue)
{
	ATLASSERT(m_hKey != NULL);
	return ::RegSetValueEx(m_hKey, pszValueName, NULL, REG_DWORD, reinterpret_cast<const BYTE*>(&dwValue), sizeof(DWORD));
}

LONG CRegKey::SetStringValue(__in_z_opt LPCTSTR pszValueName, __in_z LPCTSTR pszValue, __in DWORD dwType)
{
	ATLASSERT(m_hKey != NULL);
...
#pragma prefast(push)
#pragma prefast(suppress:__WARNING_REALLOCLEAK, "prefast bug 538")
...
ATLAPI AtlModuleRegisterTypeLib(_ATL_MODULE* pM, LPCOLESTR lpszIndex)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr) && (BSTR(bstrHelpFile) != NULL) && (lstrlenW(lpszModule) < _MAX_PATH+1))
		{
		...
		{
			hr = ::RegisterTypeLib(pTypeLib, lpszModule, NULL);
		}
	}
	if (pTypeLib != NULL)
		pTypeLib->Release();
