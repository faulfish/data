...
public:
	...
	__declspec(noinline) virtual ~CSocketWnd() { }
#endif
...
public:
	...
	BOOL Create(UINT nSocketPort = 0, int nSocketType=SOCK_STREAM,
		...
		__in_z_opt LPCTSTR lpszSocketAddress = NULL);
...
public:
	...
	BOOL Bind(UINT nSocketPort, __in_z_opt LPCTSTR lpszSocketAddress = NULL);
	BOOL Bind (const SOCKADDR* lpSockAddr, int nSockAddrLen);

	virtual void Close();

	BOOL Connect(__in_z LPCTSTR lpszHostAddress, UINT nHostPort);
	BOOL Connect(const SOCKADDR* lpSockAddr, int nSockAddrLen);

	BOOL IOCtl(long lCommand, ULONG* lpArgument);
	...
	int SendTo(const void* lpBuf, int nBufLen,
		UINT nHostPort, __in_z_opt LPCTSTR lpszHostAddress = NULL, int nFlags = 0);
	int SendTo(const void* lpBuf, int nBufLen,
