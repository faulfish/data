...
{
	ENSURE(nIndex >= 0 && nIndex < m_nSize);
...
void CArray<TYPE, ARG_TYPE>::SetSize(INT_PTR nNewSize, INT_PTR nGrowBy)
{
	...
	else
	{
		...
		ENSURE(nNewMax >= m_nMaxSize);  // no wrap around
...
{
	ASSERT_VALID(this);
	ENSURE(this != &src);   // cannot append to itself
...
{
	ASSERT_VALID(this);
	ENSURE(nIndex >= 0);    // will expand to meet need
	ENSURE(nCount > 0);     // zero or negative size not allowed
...
{
	ASSERT_VALID(this);
	ENSURE(pNewArray != NULL);
	ASSERT_VALID(pNewArray);
	ENSURE(nStartIndex >= 0);
...
//   hash table size of (which is fairly small)
{
	...
	ASSERT(m_nCount == 0);
	ASSERT(nHashSize > 0);
	ENSURE(nHashSize <= SIZE_MAX / sizeof(CAssoc*));
