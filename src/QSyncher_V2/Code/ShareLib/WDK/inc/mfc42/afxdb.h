...
// Implementation
	...
	// long binary helpers
	LONG_PTR GetLongBinarySize(int nField);
	void GetLongBinaryData(int nField, CLongBinary& lb, LONG_PTR* plSize);
	BYTE* ReallocLongBinary(CLongBinary& lb, LONG_PTR lSizeRequired,
...
void AFXAPI AfxCopyValueByRef(__in void* pvCache, __out void* pvData,
	__out LONG_PTR* plLength, int nDataType);
...
#if _MFC_VER >= 0x0600
void AFXAPI RFX_Text(__inout CFieldExchange* pFX, __in_z LPCTSTR szName, __inout_ecount(nMaxLength) LPTSTR value,
...
void AFXAPI AfxRFXBulkDefault(__inout CFieldExchange* pFX, __in_z_opt LPCTSTR szName,
	__inout_bcount_opt(cbValue) void* pv, __inout LONG_PTR* rgLengths, int nCType, UINT_PTR cbValue);

/////////////////////////////////////////////////////////////////////////////
// Bulk Recordset Field Exchange routines

void AFXAPI RFX_Text_Bulk(__inout CFieldExchange* pFX, __in_z LPCTSTR szName,
	__deref_inout_bcount_opt(nMaxLength) LPSTR* prgStrVals, __deref_inout_opt LONG_PTR** prgLengths, int nMaxLength);
...
void AFXAPI DDX_FieldText(CDataExchange* pDX, int nIDC, CString& value,
	CRecordset* pRecordset);
#if _MFC_VER >= 0x0600
void AFXAPI DDX_FieldText(CDataExchange* pDX, int nIDC, __inout_ecount_z(nMaxLength) LPTSTR pstrValue,
...
public:
	...
	static void* PASCAL GetDataBuffer(CDBVariant& varValue, short nFieldType,
		INT_PTR* pnLen, short nSQLType, SQLULEN nPrecision);
	static INT_PTR PASCAL GetTextLen(short nSQLType, SQLULEN nPrecision);
	static LONG_PTR PASCAL GetData(CDatabase* pdb, HSTMT hstmt, short nFieldIndex,
		short nFieldType, LPVOID pvData, INT_PTR nLen, short nSQLType);
...
protected:
	...
	virtual LONG_PTR GetLBFetchSize(LONG_PTR lOldSize);     // CLongBinary fetch chunking
	virtual LONG_PTR GetLBReallocSize(LONG_PTR lOldSize);   // CLongBinary realloc chunking
