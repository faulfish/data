...
#define OLE_MAXITEMNAME_CONST 21
...
// Operations
	void Attach(__inout LPDATAOBJECT lpDataObject, BOOL bAutoRelease = TRUE);
...
public:
	...
	void GetItemName(__out_ecount(OLE_MAXITEMNAME_CONST) LPTSTR lpszItemName) const; // gets readable item name
...
public:
	BEGIN_INTERFACE_PART(PersistFile, IPersistFile)
		...
		STDMETHOD(GetCurFile)(__deref_out_z LPOLESTR*);
	...
	BEGIN_INTERFACE_PART(OleItemContainer, IOleItemContainer)
		...
		STDMETHOD(ParseDisplayName)(LPBC, __in_z LPOLESTR, ULONG*, LPMONIKER*);
		STDMETHOD(EnumObjects)(DWORD, LPENUMUNKNOWN*);
		STDMETHOD(LockContainer)(BOOL);
		STDMETHOD(GetObject)(__in_z LPOLESTR, DWORD, LPBINDCTX, REFIID, LPVOID*);
		STDMETHOD(GetObjectStorage)(__in_z LPOLESTR, LPBINDCTX, REFIID, LPVOID*);
		STDMETHOD(IsRunning)(__in_z LPOLESTR);
