...
// operations
	...
	static BOOL SetCookie(
        ...
        LPCTSTR pstrUrl, 
#endif
        LPCTSTR pstrCookieName, LPCTSTR pstrCookieData);

	static BOOL GetCookie(
#ifdef _X86_
        __in_z LPCSTR pstrUrl, 
#else
        __in_z LPCTSTR pstrUrl, 
#endif
        __in LPCTSTR pstrCookieName, __out_ecount_opt(dwBufLen) LPTSTR pstrCookieData, DWORD dwBufLen);
	...
	static BOOL GetCookie(
        ...
        LPCTSTR pstrUrl, 
#endif
        LPCTSTR pstrCookieName, CString& strCookieData);
#endif
...
// Overridables
	...
	virtual UINT Read(__out_bcount(nCount) void* lpBuf, UINT nCount);
	virtual void Write(__in_bcount(nCount) const void* lpBuf, UINT nCount);
	...
	virtual LPTSTR ReadString(__out_ecount(nMax) LPTSTR pstr, UINT nMax);
...
// Operations
public:
	BOOL AddRequestHeaders(__in LPCTSTR pstrHeaders,
...
public:
	...
	BOOL GetCurrentDirectory(__out_ecount(*lpdwLen) LPTSTR pstrDirName, __inout LPDWORD lpdwLen) const;
	BOOL GetCurrentDirectoryAsURL(__out_ecount_opt(*lpdwLen) LPTSTR pstrName, __inout LPDWORD lpdwLen) const;
...
public:
	...
	virtual BOOL FindFile(__in_opt LPCTSTR pstrName = NULL,
...
public:
	...
	virtual void Dump(CDumpContext& dc) const;
#endif
	virtual BOOL GetErrorMessage(__out_ecount(nMaxError) LPTSTR lpstrError, UINT nMaxError,
