...
// Text Functions
	virtual BOOL TextOut(int x, int y, LPCTSTR __in_ecount(nCount) lpszString, int nCount);
	virtual BOOL ExtTextOut(int x, int y, UINT nOptions, LPCRECT lpRect,
				__in_ecount(nCount) LPCTSTR lpszString, UINT nCount, __in_ecount_opt(nCount) LPINT lpDxWidths);
	virtual CSize TabbedTextOut(int x, int y, __in_ecount(nCount) LPCTSTR lpszString, int nCount,
				int nTabPositions, __in_ecount_opt(nTabPositions) LPINT lpnTabStopPositions, int nTabOrigin);
	virtual int DrawText(__in_ecount(nCount) LPCTSTR lpszString, int nCount, LPRECT lpRect,
				UINT nFormat);
...
// Printer Escape Functions
	virtual int Escape(int nEscape, int nCount, __in_ecount_opt(nCount) LPCSTR lpszInData, __out_opt LPVOID lpOutData);
...
protected:
	...
	CSize ComputeDeltas(__inout int& x, __in_ecount(nCount) LPCTSTR lpszString, __inout UINT& nCount, BOOL bTabbed,
					UINT nTabStops, __in_ecount(nTabStops) LPINT lpnTabStops, int nTabOrigin,
					__out_ecount(nCount) LPTSTR lpszOutputString, __inout_ecount(nCount) int* pnDxWidths, __out int& nRightFixup);
...
// Construction
	...
	__declspec(noinline) virtual ~CMiniDockFrameWnd() { }
#endif		
...
// Attributes
	BOOL HasFont() const;
	BOOL SetFont(__in_z LPCTSTR lpFaceName, WORD nFontSize);
...
int AFXAPI AfxLoadString(UINT nIDS, __out_ecount_z(nMaxBuf) LPTSTR lpszBuf, UINT nMaxBuf = 256);
