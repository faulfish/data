...
void AFXAPI DDX_FieldText(CDataExchange* pDX, int nIDC, CString& value,
	CDaoRecordset* pRecordset);
#if _MFC_VER >= 0x0600
void AFXAPI DDX_FieldText(CDataExchange* pDX, int nIDC, __inout_ecount_z(nMaxLen) LPTSTR pstrValue,
...
public:
	...
	virtual void Create(__in_z LPCTSTR lpszName, __in_z LPCTSTR lpszUserName,
		__in_z LPCTSTR lpszPassword);
	virtual void Append();

	virtual void Open(__in_z_opt LPCTSTR lpszName = NULL);
	virtual void Close();
...
public:
	...
	virtual BOOL GetErrorMessage(__in_ecount_z(nMaxError) LPTSTR lpszError, UINT nMaxError,
...
public:
	...
	virtual void Create(__in_z LPCTSTR lpszName,
		__in_z LPCTSTR lpszLocale = dbLangGeneral, int dwOptions = 0);

	virtual void Open(__in_z LPCTSTR lpszName, BOOL bExclusive = FALSE,
		BOOL bReadOnly = FALSE, __in_z LPCTSTR lpszConnect = _T(""));
	virtual void Close();
...
// Operations
public:
	void Execute(__in_z LPCTSTR lpszSQL, int nOptions = dbFailOnError);

	void DeleteTableDef(__in_z LPCTSTR lpszName);
	void DeleteQueryDef(__in_z LPCTSTR lpszName);

	void CreateRelation(__in_z LPCTSTR lpszName, __in_z LPCTSTR lpszTable,
		__in_z LPCTSTR lpszForeignTable, long lAttributes,
		__in_z LPCTSTR lpszField, __in_z LPCTSTR lpszForeignField);
	void CreateRelation(CDaoRelationInfo& relinfo);
	void DeleteRelation(__in_z LPCTSTR lpszName);
...
public:
	...
	virtual void Create(__in_z LPCTSTR lpszName, long lAttributes = 0,
		__in_z_opt LPCTSTR lpszSrcTable = NULL, __in_z_opt LPCTSTR lpszConnect = NULL);
	virtual void Append();

	virtual void Open(__in_z LPCTSTR lpszName);
	virtual void Close();
...
// Operations
public:
	void CreateField(__in_z LPCTSTR lpszName, short nType, long lSize,
		long lAttributes = 0);
...
public:
	...
	virtual void Create(__in_z_opt LPCTSTR lpszName = NULL,
		__in_z_opt LPCTSTR lpszSQL = NULL);
	virtual void Append();
...
public:
	...
	virtual void Open(int nOpenType = AFX_DAO_USE_DEFAULT_TYPE,
		__in_z_opt LPCTSTR lpszSQL = NULL, int nOptions = 0);
...
public:
	...
	BOOL Seek(__in_z LPCTSTR lpszComparison, COleVariant* pKey1,
		COleVariant* pKey2 = NULL, COleVariant* pKey3 = NULL);
	BOOL Seek(__in_z LPCTSTR lpszComparison, COleVariant* pKeyArray, WORD nKeys);
