...
// Don't inline when building the DLLs
#define __IA64__AFX_NO_INLINE __declspec(noinline)
#else
#define __IA64__AFX_NO_INLINE
...
protected: \
	...
	{ \
	public: \
		__IA64__AFX_NO_INLINE class_name##Factory(REFCLSID clsid, CRuntimeClass* pRuntimeClass, \
...
#if defined(_AFX_DLL_IMPL) && !defined(_X86_)
	__declspec(noinline) virtual ~CControlRectTracker() { }
#endif
...
protected:
...
#if defined(_AFX_DLL_IMPL) && !defined(_X86_)
	__declspec(noinline) virtual ~CControlDataSource() { }
#endif
	...
	void GetUserType(__out_ecount_z(256) LPTSTR pszUserType);
...
public:
	...
	BEGIN_INTERFACE_PART(OleObject, IOleObject)
		...
		STDMETHOD(GetUserType)(DWORD, __deref_out_z LPOLESTR*);
...
BOOL AFXAPI AfxVerifyLicFile(HINSTANCE hInstance, __in_z_opt LPCTSTR pszLicFileName,
	__in_z_opt LPCOLESTR pszLicFileContents, UINT cch=-1);
