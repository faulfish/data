...
- (ASICachePolicy) getCachePolicy {
   
    ASICachePolicy policy = ASIUseDefaultCachePolicy;
    
    NSDictionary *_cachePolicy = [NSDictionary new];
    //Get the user documents directory
    NSString *documentsDirectory = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject];
    //Create a path to save the details
    NSString *backedUpUserDefaultsPath = [documentsDirectory stringByAppendingPathComponent:cachePolicyName];
    ...
    BOOL fileExists = [[NSFileManager defaultManager] fileExistsAtPath:backedUpUserDefaultsPath];
    if(fileExists) {
        _cachePolicy = [NSDictionary dictionaryWithContentsOfFile:backedUpUserDefaultsPath];
        NSData* data = nil;
        data = [_cachePolicy objectForKey:@"_cachePolicy"];
        int value1 = *(int*)([data bytes]);
        policy = value1;
    }
...
- (void)startSynchronous
{
    ...
    //ASICachePolicy prviousASICachePolicy = self.cachePolicy;
    self.cachePolicy = [self getCachePolicy];
    
    ASIFormDataRequest *_super = (ASIFormDataRequest *)self;
    NSMutableArray *postData = [_super _postData];
    //NSDictionary* data = (NSDictionary*)[postData objectAtIndex:0];
    NSMutableString *postURL = [NSMutableString new];
    [postURL appendString:[self.url absoluteString]];
    ...
    for (NSDictionary* dict in postData) {
        for ( id key in dict )
        {
            ...
            [postURL appendString:str];
            [postURL appendString:@":"];
            str = (NSString*)[dict objectForKey:key];
            [postURL appendString:str];
            [postURL appendString:@";"];
        }
    }
    ...
    {
        NSLog( @"%@: %@", key, [self.requestHeaders objectForKey:key] );

        NSString *str = (NSString*)key;
        if ([str compare:@"API"] == NSOrderedSame) {
            [postURL appendString:str];
            [postURL appendString:@":"];
            str = (NSString*)[self.requestHeaders objectForKey:key];
            [postURL appendString:str];
            [postURL appendString:@";"];
        }
    }
    ...
    //Disable cache for implement
    if(self.cachePolicy & ASIDoNotReadFromCacheCachePolicy) {
    }
    else {
        self.rawResponseData = [_cache objectForKey:postURL];
    }
    
    if(self.rawResponseData == nil) {
        ...
        if(self.cachePolicy & ASIDoNotWriteToCacheCachePolicy) {
        }
        else {
            if ([self.rawResponseData length] > 0 && self.responseStatusCode == 200) {
                [_cache setObject:self.rawResponseData forKey:postURL];
                [self save];
            }
        }
    ...
    [self save:ASIUseDefaultCachePolicy];//set to default
}
