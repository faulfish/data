...
@implementation Shape_UDP_Sender

- (id) initWithSocket : (NSInteger) port targetip:(NSString*) targetip targetport:(NSInteger) targetport {
    self = [super init];
    
    NSError *error = nil;
    //m_sender = new UDPMulticastSender(groupIP, groupPort, socketPort);
...
- (void) send:(NSData*) data {
    ...
    [udpSocket sendData:data toHost:groupIP port:groupPort withTimeout:-1 tag:0];
}
