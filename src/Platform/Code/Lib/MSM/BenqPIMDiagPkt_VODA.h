#if !defined _PIM_DIAG_VODA_H_
#define _PIM_DIAG_VODA_H_
...
typedef enum
{
	...
	/* 26 ~ 30 reserved for Topaz */
	BENQDIAG_CODECRC = 31,
	/* 32 ~ 40 reserved for Topaz */

	BENQDIAG_PB_TOOL_V3 = 41,
	BENQDIAG_PIM_MMS_V2 = 42,
...
#define PIM_NV_MAX_PHONE_DIAIGS			20
#define PIM_MAX_EMAIL_LEN               128
#define PIM_MAX_EMAIL_STR_LEN           PIM_MAX_EMAIL_LEN / 2
#define PIM_GUI_FS_FILENAME_MAX_LENGTH_P PIM_FS_FILENAME_MAX_LENGTH_P / 2
#define PIM_BOOK_REMARK_LENGTH          100
#define PIM_BOOK_REMARK_STR_LENGTH      PIM_BOOK_REMARK_LENGTH / 2
#define PIM_NV_BENQ_PB_STREET_LEN	    100
#define PIM_NV_BENQ_PB_STREET_STR_LEN   PIM_NV_BENQ_PB_STREET_LEN / 2
#define	PIM_NV_BENQ_PB_CITY_LEN			64
#define	PIM_NV_BENQ_PB_CITY_STR_LEN	PIM_NV_BENQ_PB_CITY_LEN / 2
#define PIM_NV_BENQ_PB_STATE_LEN		64
#define PIM_NV_BENQ_PB_STATE_STR_LEN	PIM_NV_BENQ_PB_STATE_LEN / 2
#define PIM_NV_BENQ_PB_COUNTRY_LEN		40
#define PIM_NV_BENQ_PB_COUNTRY_STR_LEN	PIM_NV_BENQ_PB_COUNTRY_LEN / 2
#define PIM_NV_BENQ_PB_POSTAL_LEN    20
...
{
	WORD	address;		                                      /* record real location */
	BYTE	letters[PIM_NV_MAX_NAME_STR];                         /* phone name */
	BYTE	lettersNum;		                                      /* phone name length */
	BYTE	digits[PIM_NV_MAX_PHONE_NUM][PIM_NV_MAX_DIAL_DIGITS];  /* phone number (ASCII code) */
	BYTE	digitsNum[PIM_NV_MAX_PHONE_NUM];		              /* phone number length */
	BYTE    phoneType[PIM_NV_MAX_PHONE_NUM];		              /* phone type */
    ...
    BYTE    szRemark[(PIM_BOOK_REMARK_STR_LENGTH+1)*2]; 	
	BYTE    streetNum;
	BYTE	street[PIM_NV_BENQ_PB_STREET_LEN];
	BYTE    cityNum;
	BYTE	city[PIM_NV_BENQ_PB_CITY_LEN];
	BYTE    stateNum;
	BYTE	state[PIM_NV_BENQ_PB_STATE_LEN];
	BYTE    countryNum;
	BYTE	country[PIM_NV_BENQ_PB_COUNTRY_LEN];
	BYTE	postalNum;	
	BYTE	postal[PIM_NV_BENQ_PB_POSTAL_LEN];	
}benqdiag_pb_info_type_v2;
...
typedef struct
{
    ...
    union{
		...
		benqdiag_pb_info_type_v2	pb_info_v2;
...
#define  PIM_UI_WMS_CONCATENATE_MAX   10
...
typedef enum
{
	MMS_MESSAGE_CLASS_NOT_PRESENT		=	0,
...
typedef enum
{
    MSG_STS_NOT_PRESENT = -1,

	MSG_STS_RECV_NOT_READ,
	MSG_STS_RECV_READ,

    MSG_STS_EDIT_NOT_SEND,
    MSG_STS_EDIT_SEND,    

	MSG_STS_RECV_FETCH_FAILED,
	MSG_STS_RECV_NOT_FETCH,

    MSG_STS_COMPLETED,
    MSG_STS_NOT_COMPLETED,
    MSG_STS_INVALID,
...
//BenQ Joe Lo {
//ADD: For new MC structures!
/***********************************************************************************
 * MMS v2 Data                                                                     *
 ***********************************************************************************/
#define   BENQDIAG_MMS_STATUS_OK     0
#define   BENQDIAG_MMS_STATUS_FAIL   0xFFFF

enum 
{
    BENQDIAG_MMS_PROFILE,             /* Init pt service book module */
    BENQDIAG_MMS_SAVE,                /* Add new record or modify one record */    
    BENQDIAG_MMS_VIEW,                /* Read one record by location */
    BENQDIAG_MMS_DELMSG,              /* Delete one record by location */
    BENQDIAG_MMS_SEND,                     
    BENQDIAG_MMS_LIST,
    BENQDIAG_MMS_CLRBOX,
	BENQDIAG_MMS_COPY,
	BENQDIAG_MMS_NUM_IN_BOX,
	BENQDIAG_MMS_CHECK_CONNECTION,
	BENQDIAG_MMS_READ_REPORT,
	
    BENQDIAG_NUMMMSOPERATIONS         /* number of option events  */
};

typedef struct
{
	BYTE    charset[4];
}benqdiag_mc_TextParamS;

typedef struct
{
	BYTE    boundary[78];
	
//#if defined MC_SUPPORT_DRM_PHASE1 || defined MC_SUPPORT_DRM_PHASE2
	BYTE    typeString[200];
//#endif

}benqdiag_mc_DrmParamS;

typedef struct
{
	BYTE    type[30];
	BYTE    start[30];
}benqdiag_mc_MultiRelParamS;

typedef union
{
	benqdiag_mc_TextParamS		  textParameter;
	benqdiag_mc_DrmParamS		  DrmParameter;
	benqdiag_mc_MultiRelParamS	  MultiRelParameter;
}benqdiag_mc_MmsParameterU;
...
typedef struct
{
	...
	benqdiag_mc_MmsParameterU	Parameter;
}benqdiag_mc_MmsContentTypeS;
...
typedef struct
{
    ...
    benqdiag_mc_MmsContentTypeS    contentType;    /* Media object (attachment) 's type */
...
typedef struct
{
    ...
    benqdiag_mc_MmsEntryS    aMmsEntry[MMS_MAX_ENTRY_NUM];    /* Info of each media object */
}benqdiag_mc_ListFileInfoS;
...
/*    mc_DcsE		dcs;   */
	/*
	 *	Now subject data just be UCS2 string..  Pesalo Liu [1/23/2005]
	 */
    DWORD		len;    
...
typedef struct
{
    ...
    BYTE     sentTime[6];    //mc_BcdTime, YY.MM.DD.HH.MM.SS
...
typedef struct 
{
	...
	BYTE	maxAddrLen;
	BYTE	maxAliasLen;
	BYTE	maxFilenameLen;		// MC_MAX_FILENAME_LEN
}benqdiag_mms_PROFILE_info_s_type;
...
typedef struct
{
	...
	BYTE	 PIMSMILNameLen;
}benqdiag_mms_SAVE_req_parameter_s_type;
...
{
	DWORD    msgID;
}benqdiag_mms_VIEW_req_parameter_s_type;
...
{
	DWORD    msgID;
}benqdiag_mms_DELMSG_req_parameter_s_type;

/* SEND */
typedef struct 
{
	DWORD	msgID;
}benqdiag_mms_SEND_req_parameter_s_type;
...
typedef struct 
{
	DWORD    boxID;
	WORD     index;
}benqdiag_mms_LIST_req_parameter_s_type;
...
{
	DWORD    boxID;
}benqdiag_mms_CLRBOX_req_parameter_s_type;
...
typedef struct
{
	...
	WORD     msgNum;
	DWORD    boxID;
}benqdiag_mms_COPY_req_parameter_s_type;

/* NUM in BOX */
typedef struct
{
	DWORD    boxID;
}benqdiag_mms_NumInBox_req_parameter_s_type;
...
{
	DWORD    msgID;
}benqdiag_mms_ReadReport_req_parameter_s_type;
...
typedef struct {
    ...
    union
	{
		benqdiag_mms_SAVE_req_parameter_s_type		save_req;
		benqdiag_mms_SEND_req_parameter_s_type		send_req;
		benqdiag_mms_LIST_req_parameter_s_type		list_req;
		benqdiag_mms_VIEW_req_parameter_s_type		view_req;
		benqdiag_mms_DELMSG_req_parameter_s_type	delete_msg_req;
        benqdiag_mms_CLRBOX_req_parameter_s_type	clear_box_req;
		benqdiag_mms_COPY_req_parameter_s_type		copy_rep;
		benqdiag_mms_NumInBox_req_parameter_s_type	NumInBox_req;
		benqdiag_mms_ReadReport_req_parameter_s_type	read_report_req;
	}data;
...
typedef struct {
    ...
    union
	{
		benqdiag_mms_PROFILE_info_s_type			profile_info;
