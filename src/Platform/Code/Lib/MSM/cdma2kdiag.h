...
/*===========================================================================
  ...
  $Header:   //BQT/VM67/Prjbase3/QSyncher_V2/PlatformTool/Code/Lib/MSM/cdma2kdiag.h.-arc   1.1   Apr 01 2009 14:19:22   Kevin Tai  $
...
typedef PACKED struct {
  ...
  word    rf_mode;                /* 0->Analog,  1->CDMA Cellular,  2->CDMA PCS 
                                     3->Sleep mpdes, 4->GPS, 5->HDR            */
...
typedef PACKED struct
{
  ...
  word nv_stat;        /* NV write status if OK_DONE */
...
typedef PACKED struct
{
  byte cmd_code;                               /* Command code */
  byte rl_status;                              /* Status of block, as above */
  word nv_stat;                   /* Status returned by NV */
  byte seq_num;                                /* Sequence number */
  byte more;                                   /* More to come? */
