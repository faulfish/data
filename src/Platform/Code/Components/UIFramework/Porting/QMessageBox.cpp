...
void QMessageBox::SetBackground(int nBtnCount)
{
	...
	StringFormat format;
	SizeF  szLayoutSize((REAL) ( (nBtnCount < 3) ? 303-89 : 353-89 ), 100-48 );
	SizeF  szBound;
	INT codepointsFitted;
	graphics.MeasureString(m_pILocalization->GetString(m_strWarningText).c_str(),(INT)m_pILocalization->GetString(m_strWarningText).length(),&myFont,szLayoutSize,&format,&szBound,&codepointsFitted,&m_nNumberOfTextLines);
...
void QMessageBox::OnBnClickedOK() //Called after keying in "Delete" key on keyboard, not the "Delete button" on the view.
{
	g_pThis->EndDialog(QIDOK);
}
	...
	case WM_KEYDOWN:
		{
			...
			case VK_ESCAPE:
				...
				break;
			case VK_RETURN:
				if(m_nID == QMB_OK || m_nID == QMB_OKCANCEL)
					OK();
