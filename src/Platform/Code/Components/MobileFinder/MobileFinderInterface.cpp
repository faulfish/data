...
namespace QSYNC_LIB
{
	...
	public:	
		virtual bool Register(INotify* pNotify)
		{
			...
			if(pNotify)
			{
				...
				bRes = bRes && m_DectectDevice.Register(pNotify,DEVICE_DIAG_CONNECT);
				bRes = bRes && m_DectectDevice.Register(pNotify,DEVICE_DIAG_DISCONNECT);
			}
			return bRes;
		}
		virtual bool Unregister(INotify* pNotify)
		{
			...
			if(pNotify)
			{
				...
				bRes = bRes && m_DectectDevice.Unregister(pNotify,DEVICE_DIAG_CONNECT);
				bRes = bRes && m_DectectDevice.Unregister(pNotify,DEVICE_DIAG_DISCONNECT);
			}
			return bRes;
		}
	...
	CDCINTERFACE_DLLEXT void DeleteDeviceFinder()
	{
		...
		{
			//It will be execuate on delete g_pMobileFinderInterface;
