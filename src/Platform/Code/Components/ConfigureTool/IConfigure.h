...
#define		CONFIGURETOOL_DLL_DECLSPEC	extern "C"	__declspec(dllexport)
#else
#define		CONFIGURETOOL_DLL_DECLSPEC	extern "C"	__declspec(dllimport)
#endif
...
#define _TC(x) x
	...
	public:
		...
		CCfgType& operator+(const TYPE& Value)
		{
			if(m_pICfg->SetCfgVar(m_strKeyName,m_LocalVar + Value))
				m_LocalVar = m_LocalVar +Value;
			return *this;
		}
