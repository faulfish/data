...
//Sample code of CZipArchive
/*
bool Unzip(const CString& strZippedFileName,const CString& strUnZipDestFolder)
{
	bool bRes = true;
	//unzip the *.sdq file and store files in strUnZipDestFolder folder
	CZipArchive zip;
	zip.Open(strZippedFileName, CZipArchive::openReadOnly);
	for(WORD i = 0; i< zip.GetNoEntries(); i++) 
	{
		bRes = zip.ExtractFile(i, strUnZipDestFolder);
		if(bRes == false) 
		{
			m_pLogMessage->SendLogString(_T("	Extracting zip file failed!\n"));
			CString strError = _T("\n		Extract file failed. Please try again or choose another theme file.");
			MessageBox(AfxGetApp()->m_pMainWnd->GetSafeHwnd(), strError, _T("Error"), MB_ICONERROR);
			break;
		}
	}
	zip.Close();
