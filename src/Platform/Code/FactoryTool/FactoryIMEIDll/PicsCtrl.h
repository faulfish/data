...
public:
	...
	short GetSIMData(short SIMType, const char IMEI_Array[16],const char FSN_Array[33],const char *SIMFileName, char SIM_Data_Array[1153], short *SIM_Data_Length);
	short QueryFSNCode(const char IMEI_Array[16],char FSN_Array[33]);
	short GetFSNCode(const char IMEI_Array[16], const char FSN_Array[33]);
	...
	unsigned char Initial_PICSInterfaceDLL();
	CPicsCtrl();
