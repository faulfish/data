...
unsigned char CPicsCtrl::Initial_PICSInterfaceDLL()
{
	...
	hresult=CLSIDFromProgID(OLESTR("PICSInterface.clsPICSInterface"), &clsid);
	hresult=CoCreateInstance(clsid,NULL,CLSCTX_INPROC_SERVER,__uuidof(_clsPICSInterface),(LPVOID *) &PICS_Interface);
	if(FAILED(hresult))
	{
		AfxMessageBox(_T("Register PICS Interface DLL fail!!"));
		return FALSE;
	}
	CheckPlatformInPICS("Qualcomm");
    short result=ConnectPICS();
	if(result)
	{
		char Temp_Global[2048]="\0";
		sprintf(Temp_Global,"Conncet PICS fail!!\nReturn value:%d",result);
		AfxMessageBox(Temp_Global);
		return FALSE;
	}
...
{	
	...
	if(result)
		return result;
	_temp = _SPC.Copy();
	_stprintf((char *)SPC_Array , _T("%s"), (LPCTSTR)_temp);
	_SPC.Empty();
	return result;
	/*************************************************************
	MID_RET_SUCCESS	' 0 = Success.
...
short CPicsCtrl::GetFSNCode(const char IMEI_Array[], const char FSN_Array[])
{
	if(!Initial_PICSInterfaceDLL())
	{
		return 1;
	...
	if(result)
		return result;
	_temp = _FSN.Copy();
	_stprintf((char *)FSN_Array , _T("%s"), (LPCTSTR)_temp);
	_FSN.Empty();
	return result;
	/*************************************************************
...
short CPicsCtrl::QueryFSNCode(const char IMEI_Array[], char FSN_Array[])
{
	if(!Initial_PICSInterfaceDLL())
	{
		return 1;
...
short CPicsCtrl::GetSIMData(short SIMType, const char IMEI_Array[],const char FSN_Array[],const char *SIMFileName,char SIM_Data_Array[], short *SIM_Data_Length)
{
										 ...
										 char Phone_Lock_Code_Array[])
{
	...
	_stprintf((char *)Phone_Lock_Code_Array , _T("%s"), (LPCTSTR)_temp);
