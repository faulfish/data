...
unsigned char CPicsCtrl::Initial_PICSInterfaceDLL()
{
    ...
    HRESULT hresult;
	/*UINT Result;
	CString ExecutableFilePath=Current_Directory+"\\Copy_REGTOOLS_REGTLIB.bat";
	...
	{
	    Message_String="Copy regtools.reg and REGTLIB.exe file to C:\\windows\\system32 fail!!";
		AfxMessageBox(Message_String);
		return FALSE;
	}

	ExecutableFilePath=Current_Directory+"\\REGTLIB.EXE";
	Result=WinExec(ExecutableFilePath,SW_SHOW);
	if((Result==0)||
	   (Result==ERROR_BAD_FORMAT)||
	   (Result==ERROR_FILE_NOT_FOUND)||
