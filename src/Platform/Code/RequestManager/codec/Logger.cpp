...
int LogInformation(int nShowLevel, LPCTSTR format,...)
{
	...
	if (hLogFile) {
		...
		WriteFile(hLogFile,szMsg,(DWORD)_tcslen(szMsg),&dwOut,0L);
...
int  LogBuffer(int nShowLevel, const void *pvBuf, int nLen){
	...
	LogInformation(nShowLevel,"The %d bytes buffer content is: %s", nLen, szTarget);
	delete[] szTarget;
	szTarget = NULL;
	return RUN_SUCCESSFULLY;
}
...
int LogCOMFuncBuffer(int nShowLevel, LPCTSTR szCOM, LPCTSTR szFunc, const void *pvBuf, int nLen){
	...
	LogInformation(nShowLevel,"%s\t%s\tThe %d bytes buffer content is: %s", szCOM, szFunc,nLen,szTarget);
	delete[] szTarget;
	szTarget = NULL;
	return RUN_SUCCESSFULLY;
