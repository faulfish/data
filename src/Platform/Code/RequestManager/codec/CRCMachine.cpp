...
*/
DWORD	CCRCMachine::s_dwCRC30Table[ CRC_TAB_SIZE ] = {
...
// calculated 8 bits at a time, MSB first.
//
WORD CCRCMachine::s_wCRC16Table[ CRC_TAB_SIZE ] = {
...
WORD	CCRCMachine::s_wCRC16LTable[ CRC_TAB_SIZE ] = {
...
//WORD CCRCMachine::CRC16LCalculation(LPTSTR szBuffer, WORD wBitLen);
//WORD CCRCMachine::CRC16Calculation(LPTSTR szBuffer, WORD wBitLen);
//DWORD CCRCMachine::CRC30Calculation(LPTSTR szBuffer, DWORD dwBitLen);

/////////////////////////////////////////////////////////////////////////////
// CCRCMachine class operations

DWORD CCRCMachine::CRC30Calculation(LPTSTR szBuffer, DWORD dwBitLen)
{
	DWORD dwData, dwCRC30;

	/* Generate a CRC-30 by looking up the transformation in a table and
	** XOR-ing it into the CRC, one byte at a time.
	*/
	for(dwCRC30 = CRC30SEED ; dwBitLen >= 8; dwBitLen -= 8, szBuffer++) 
	{
		dwCRC30 = s_dwCRC30Table[ ((dwCRC30 >> (30 - 8)) ^ ((BYTE)(*szBuffer))) & 0xff ] ^ (dwCRC30 << 8);
	}
	...
	*/
	if (dwBitLen != 0) 
	{
		dwData = ((DWORD) (BYTE)(*szBuffer)) << (30-8); /* Align dwData MSB with CRC MSB */

		while (dwBitLen-- != 0) 
		{
			if ( ((dwCRC30 ^ dwData) & (1L << 29)) != 0 ) 
			{       /* Is MSB of XOR a 1 */

				dwCRC30 <<= 1;                   /* Left shift CRC          */
				dwCRC30 ^= CRC30POLYNOMIAL;    /* XOR polynomial into CRC */
			...
			dwData <<= 1;                       /* Left shift dwData         */
		}
	}

	dwCRC30 = ~dwCRC30;                     /* 1's complement the CRC     */

	return( dwCRC30 & 0x3FFFFFFF );        /* mask the result to 30 bits */

}

WORD CCRCMachine::CRC16Calculation(LPTSTR szBuffer, DWORD dwBitLen)
{
	WORD wData, wCRC16;
	...
	** XOR-ing it into the CRC, one byte at a time.
	*/
	for (wCRC16 = CRC16SEED ; dwBitLen >= 8; dwBitLen -= 8, szBuffer++) 
	{
		wCRC16 = s_wCRC16Table[ (wCRC16 >> (16 - 8)) ^ ((BYTE)(*szBuffer)) ] ^ (wCRC16 << 8);
	}

	/* Finish calculating the CRC over the trailing wData bits
	**
	** XOR the MS bit of wData with the MS bit of the CRC.
	** Shift the CRC and wData left 1 bit.
	** If the XOR result is 1, XOR the generating polynomial in with the CRC.
	*/
	if (dwBitLen != 0) 
	{
		wData = ((WORD)(BYTE)(*szBuffer)) << (16-8); /* Align wData MSB with CRC MSB */

		while (dwBitLen-- != 0) 
		{
			if ( ((wCRC16 ^ wData) & ((WORD) 1 << 15)) != 0 )
			{ /* Is MSB of XOR a 1 */

				wCRC16 <<= 1;                   /* Left shift CRC          */
				wCRC16 ^= CRC16POLYNOMIAL;    /* XOR polynomial into CRC */
			...
			wData <<= 1;                       /* Left shift wData         */
		}
	}

	return( ~wCRC16 );            /* return the 1's complement of the CRC */

}


WORD CCRCMachine::CRC16LCalculation(LPTSTR szBuffer, DWORD dwBitLen)
{
	WORD wData, wCRC16;
	...
	** XOR-ing it into the CRC, one byte at a time.
	*/
	for (wCRC16 = CRC16LSEED ; dwBitLen >= 8; dwBitLen -= 8, szBuffer++) 
	{
		wCRC16 = s_wCRC16LTable[ (wCRC16 ^ (*szBuffer)) & 0x00ff ] ^ (wCRC16 >> 8);
	}
	...
	*/
	if (dwBitLen != 0) 
	{
		wData = ((WORD)(BYTE)(*szBuffer)) << (16-8); /* Align data MSB with CRC MSB */
		while (dwBitLen-- != 0) 
		{
			if ( ((wCRC16 ^ wData) & 0x01) != 0 )
			{   /* Is LSB of XOR a 1 */
				wCRC16 >>= 1;                   /* Right shift CRC         */
				wCRC16 ^= CRC16LPOLYNOMIAL;  /* XOR polynomial into CRC */
			} 
			else 
			{
				wCRC16 >>= 1;                   /* Right shift CRC         */
			}
			wData >>= 1;                       /* Right shift wData        */
		}
	}
	return( ~wCRC16 );            /* return the 1's complement of the CRC */
}

// Block calculation for CRC checking.
// dwOption meaning:
//	1: Start
//	2: Continue block
//	3: End block
...
DWORD CCRCMachine::CRC30BlockCal(LPTSTR szBuffer, DWORD dwBitLen, DWORD dwOption){
	_ASSERTE( (dwBitLen > 0) && ((dwBitLen % 8) == 0) );
	static DWORD dwCRC30 = CRC30SEED;
	...
	*/
	...
	for(; dwBitLen >= 8; dwBitLen -= 8, szBuffer++) 
	{
		dwCRC30 = s_dwCRC30Table[ ((dwCRC30 >> (30 - 8)) ^ ((BYTE)(*szBuffer))) & 0xff ] ^ (dwCRC30 << 8);
	}
