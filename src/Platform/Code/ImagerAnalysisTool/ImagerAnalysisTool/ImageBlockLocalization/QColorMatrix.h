...
//
// Version 1.1:
// SetTint() method added.
// copy ctor and operator= now work with ColorMatrix (used to be QColorMatrix).
...
public:
	QColorMatrix()							{ Reset(); }
	QColorMatrix(const ColorMatrix& m)		{ Copy(m); }
	~QColorMatrix()							{ }

	QColorMatrix& operator=(const ColorMatrix& m)
	...
	// Scale tint given by phi (-180... 180; 0 == blue); amount: -1.0f... 1.0f (0.0f neutral).
