...
Status QColorMatrix::SetTint(REAL phi, REAL amount)
{
	InitHue();
	ASSERT(initialized);

	// Rotate hue.
	Status s = RotateHue(phi);
//	if (s != Ok) return s;		// Can never happen

	// Scale blue
	s = Scale(1.0f, 1.0f, 1.0f + amount, 1.0f, MatrixOrderAppend);
//	if (s != Ok) return s;

	// Rotate hue back.
	return RotateHue(- phi);
}

void QColorMatrix::Copy(const ColorMatrix& rhs)
{
	::CopyMemory(m, rhs.m, 25 * sizeof(REAL));
}
