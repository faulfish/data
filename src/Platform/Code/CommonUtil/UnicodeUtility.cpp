...
namespace QSYNC_LIB 
{
	...
	int toInt( TSTRING const& string )
	{
#ifdef _UNICODE
		wistringstream cvt( string ) ;
#else
		istringstream cvt( string ) ;
#endif
		int result ;
		cvt >> result ;
		return result ;
	...
	void MakeLower(TSTRING &str)
	{
		std::transform(str.begin(),str.end(),str.begin(),tolower);
	}

	void TrimLeft(TSTRING& str, const TSTRING& chars2remove)
	{
		if (!str.empty())
		{
			TSTRING::size_type pos = str.find_first_not_of(chars2remove);
	...
	void TrimRight(TSTRING& str, const TSTRING& chars2remove)
	{
		if (!str.empty())
		{
			TSTRING::size_type pos = str.find_last_not_of(chars2remove);
	...
	{
		TSTRING::size_type nFindIndex = strOriginal.find(strSeperator);
		if(nFindIndex != TSTRING::npos)
		{
			strToken1 = strOriginal.substr(0,nFindIndex);
			strToken2 = strOriginal.substr(nFindIndex+strSeperator.size());
		}
		else
		{
			strToken1 = strOriginal;
			strToken2.clear();
		}
	}
...
#else
		::AtlHexDecode(strNewHex.c_str(),static_cast<int>(strNewHex.size()),byteBuffer,&nBufferLen);
#endif
		DWORD dwValue = 0;
		DWORD dwDegree = 1;
