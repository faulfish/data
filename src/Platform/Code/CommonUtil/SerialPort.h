...
public:
	...
	CSerialPort(CSerialPortMode nMode = CSPATCommandMode):m_nCSPMode(nMode),m_hComm(NULL),m_hThread(NULL),m_ptBuffer(new CBuffer){}
	CSerialPort(const TSTRING& portname,CSerialPortMode nMode = CSPATCommandMode):m_nCSPMode(nMode),m_portname(portname),m_hComm(NULL),m_hThread(NULL),m_ptBuffer(new CBuffer){OpenPort(m_portname);}
