...
#define _TC(x) "x"
	...
	int SearchWord(const TSTRING& strFilePath, const TSTRING& strSearchWord, int nLimit = -1/*no limit*/);  // return word location, if can't be found, return -1.
	...
	bool ByteToChar(BYTE *pucHexBcdBuf, int iHexBcdBufSize, CStringA& strBuffer, bool bNeedSpace = false); // 0xAB = A,B
	bool CharToByte(const char* szInput, int nInputSize, BYTE* szOutput/*OutputSize = szInputSize/2 + szInputSize%2 */);// AB CD E = 0xAB 0xCD 0xE0
}
