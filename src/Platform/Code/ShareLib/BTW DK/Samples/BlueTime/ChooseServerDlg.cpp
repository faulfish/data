...
public:
    ...
    CBdInfo(BD_ADDR bd_addr, BD_NAME bd_name) : m_Psm(0), m_isServer(FALSE), m_Name(bd_name) { memcpy (m_BdAddr, bd_addr, BD_ADDR_LEN); }
    ~CBdInfo() { ; }
...
BEGIN_MESSAGE_MAP(CChooseServerDlg, CDialog)
    ...
    ON_BN_CLICKED(IDC_BONDING_BUTTON, OnBondingButton)
    //}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CChooseServerDlg::OnClickServerList(NMHDR* pNMHDR, LRESULT* pResult) 
{
    ...
    if (pos != NULL)
    {
        ...
        // has been done on the device and a Time Service was 
        // found.
        //
...
// demonstrates stack status change callback 
void CChooseServerDlg::OnStackStatusChange(STACK_STATUS new_status)
{
    CString strMsg;
    strMsg.Format("StackStatus: %d",new_status);
    SetDlgItemText(IDC_STATUS_TEXT, strMsg);
}
...
void CChooseServerDlg::DoAddDevice(BD_ADDR p_bda, BD_NAME bd_name)
{
    ...
    for (int x=0; x < item_count; x++)
    {
        ...
        if ( p_CmpInfo->isBdAddrEqual(p_bda))
        {
            ...
            // item text.
            if ((p_CmpInfo->m_Name.GetLength() == 0) || (p_CmpInfo->m_Name == "<No Name>"))
            {
    ...
    SetDlgItemText(IDC_STATUS_TEXT, "Select a device and click Discover to find Time service");
}
...
UINT CChooseServerDlg::DiscoverServices(LPVOID pParam) 
{
    ...
    if (pos != NULL)
    {
        ...
        if ( ! p_Dlg->StartDiscovery(p_Info->m_BdAddr, p_Dlg->m_pServiceGuid) )
        {
            // Discovery failed to start.  This is usually due to
            // discovery already running.
            //
            p_Dlg->SetDlgItemText(IDC_STATUS_TEXT, "Discovery Failed for BDA:  " + p_Info->DeviceAsString());
        }
        else
        {
            //
            ...
            if (! p_Dlg->m_isDialogClosing)
            {
                ...
                if ( p_Dlg->ReadDiscoveryRecords(p_Info->m_BdAddr, 1, &Sdp_Record, p_Dlg->m_pServiceGuid) )
                {
                    ...
                    if ( Sdp_Record.FindL2CapPsm(&psm) )
                    {
                        p_Info->m_Psm = psm;
                        ...
                        // test for long INT attributes
                        SDP_DISC_ATTTR_VAL aValue = {0};
                        if (Sdp_Record.FindAttribute(0x05FF, &aValue))
                        {
                            // verify it's correct
                            if((aValue.elem[0].val.u128.uHigh64 != 0x8001020304050607) || (aValue.elem[0].val.u128.uLow64 != 0x08090a0b0c0d0e0f))
                            {
                                p_Dlg->SetDlgItemText(IDC_STATUS_TEXT, "16 byte integer attribute transfer corrupted!");
                                Sleep(10000);
                            }
                            else
                            {
                                p_Dlg->SetDlgItemText(IDC_STATUS_TEXT, "16 byte integer attribute transferred successfully");
                                Sleep(2000);
                            }
                        }

                        // test for new max attr len
                        memset(&aValue, 0, sizeof(SDP_DISC_ATTTR_VAL));
                        if (Sdp_Record.FindAttribute(0x05FE, &aValue))
                        {
                            // verify it's correct, maybe old size if running on old BTW
                            UINT16 i;
                            for (i = 0; i <= MAX_ATTR_LEN; i++)
                            {
                                if(aValue.elem[0].val.array[i] != 0x44)
                                    break;
                            }

                            if ((i != (MAX_ATTR_LEN - 1)) && (i != (MAX_ATTR_LEN_OLD - 1)))
                            {
                                p_Dlg->SetDlgItemText(IDC_STATUS_TEXT, "long attribute transfer corrupted!");
                                Sleep(10000);
                            }
                            else
                            {
                                p_Dlg->SetDlgItemText(IDC_STATUS_TEXT, "long attribute transferred successfully");
                                Sleep(2000);
                            }
                        }
                    }
...
void CChooseServerDlg::OnBondingButton() 
{
    // TODO: Add your control notification handler code here
    // TODO: Add your control notification handler code here
    CBondDialog dlg;

    // if user hasn't selected a device forget it
    POSITION pos = m_ServerList.GetFirstSelectedItemPosition();
    ...
    if (pos == NULL)
    {
        SetDlgItemText(IDC_STATUS_TEXT, "Select a device for bonding.");
        return;
    }
    else
    {
        ...
        // The dialog needs the device name and bd address.
        memcpy(dlg.m_BdAddr, p_Info->m_BdAddr, BD_ADDR_LEN);

        dlg.m_Name = p_Info->DeviceAsString();

        if (dlg.DoModal() == IDOK)
        {
            SetDlgItemText(IDC_STATUS_TEXT, "");
        }
        //
