// DownloadUIForIthacaDlg.cpp : implementation file
//
...
#define VERSION _T("1.0.0.8 for Ithaca")
...
namespace QSYNC_LIB {
	...
	CDownloadUIForIthacaDlg* g_pThis = NULL;


	CDownloadUIForIthacaDlg::CDownloadUIForIthacaDlg(CWnd* pParent /*=NULL*/):Win32DoubleBufferQDialogWidge(CDownloadUIForIthacaDlg::IDD, pParent)
	, m_nStep(0), m_bDifferentPartition(false)
	{
	...
	CDownloadUIForIthacaDlg::~CDownloadUIForIthacaDlg() {
		if(m_pLayoutManager)
	...
	void CDownloadUIForIthacaDlg::DoDataExchange(CDataExchange* pDX)
	{
		CDialog::DoDataExchange(pDX);
	}

	BEGIN_MESSAGE_MAP(CDownloadUIForIthacaDlg, CDialog)
		ON_WM_PAINT()
		ON_WM_QUERYDRAGICON()
		ON_WM_SIZING()
		ON_WM_CONTEXTMENU()
...
// CDownloadUIForIthacaDlg message handlers

	BOOL CDownloadUIForIthacaDlg::OnInitDialog()
	{
		...
		pListBox->InsertString(pListBox->GetCount(), _T("This is special version for Ithaca..."));
	...
	void CDownloadUIForIthacaDlg::OnPaint() {
		if (IsIconic()) {
	...
	HCURSOR CDownloadUIForIthacaDlg::OnQueryDragIcon() {
		return static_cast<HCURSOR>(m_hIcon);
	}

	void CDownloadUIForIthacaDlg::SetXMLUI(void) {
	...
	void CDownloadUIForIthacaDlg::SetBackground(TSTRING strBkImage) {
	...
	void CDownloadUIForIthacaDlg::WinClose() {
	...
	UINT DownloadThread(LPVOID pParam) {
		CDownloadUIForIthacaDlg* pDlg = (CDownloadUIForIthacaDlg*)pParam;
		pDlg->DownloadFunc();
		return 1;
	}

	void CDownloadUIForIthacaDlg::GetStepString(CStringA& strMsg) {
	...
	void CDownloadUIForIthacaDlg::Event(const std::string& strEvent,long nParam) {
	...
	//		CString szTime;
	//		szTime.Format(_T("%s: %02d:%02d:%02d"), pwText, t.GetHour(), t.GetMinute(), t.GetSecond());
	//		InsertStringList(szTime, _T("Downloading..."));
			delete[] pwText;
	...
	void CDownloadUIForIthacaDlg::Event(const TSTRING& strEvent, long nParam) {
	...
	void CDownloadUIForIthacaDlg::OnEventButtonClick(long nParam) {
	...
	void CDownloadUIForIthacaDlg::OnEventConnected(long nParam) {
	...
	void CDownloadUIForIthacaDlg::OnEventDisconnect(long nParam) {
		DeviceDataPt* pspDeviceData = (QSYNC_LIB::DeviceDataPt*)nParam;
		DeviceDataPt spDeviceData = *pspDeviceData; 
	...
	void CDownloadUIForIthacaDlg::OnSizing(UINT fwSide, LPRECT pRect) {
	...
	int CDownloadUIForIthacaDlg::CheckBatteryStage() {
		int nRet = IDYES;
		int nVoltage = ReadBatteryVoltage();
		if((nVoltage != 0) && (nVoltage < 3500)) {
	...
	bool CDownloadUIForIthacaDlg::SetAllImageParameters() {
		int nRet = _taccess(m_strIni.GetString(), 0);
		if(nRet == -1) return false;

	}

	void CDownloadUIForIthacaDlg::DownloadStage() {
		...
		CShareDownload6270 download(m_nSelectCom);
	...
	void CDownloadUIForIthacaDlg::FinalReboot() {
		TCHAR szBuf[16] = {0};
	...
	void CDownloadUIForIthacaDlg::DownloadFunc() {
    ...
    void CDownloadUIForIthacaDlg::BackupBRTForIthaca() {
    ...
    void CDownloadUIForIthacaDlg::RestoreBRTForIthaca() {
	...
	void CDownloadUIForIthacaDlg::Initialize() {
	...
	bool CDownloadUIForIthacaDlg::GetInfoFromME() {
	...
	bool CDownloadUIForIthacaDlg::CallBack(const char* szMsg, int nStep, int nProgress) {
		g_pThis->UpdateUIProgressBar(szMsg, nStep, nProgress);
		return true;
	}

	void CDownloadUIForIthacaDlg::UpdateStepInformation(WIN32PictureViewerWidge** pStep, int nType) {
		switch(nType) {
			case 0:
	...
	bool CDownloadUIForIthacaDlg::UpdateUIProgressBar(const char* szMsg, int nStep, int nProgress) {
	...
	void CDownloadUIForIthacaDlg::OnContextMenu(CWnd* pWnd, CPoint point) {
		CMenu PopMenu;
	...
	void CDownloadUIForIthacaDlg::OnBackGround() {
	...
	void CDownloadUIForIthacaDlg::OnImage() {
		TCHAR szPath[MAX_PATH];
		::GetCurrentDirectory(MAX_PATH, szPath);
		CFileDialog fd(TRUE, NULL, NULL, OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT, _T("JPG File(*.jpg)|*.jpg|PNG File(*.png)|*.png|Bitmap File(*.bmp)|*.bmp||"));
		if(fd.DoModal() == IDOK) {
	...
	void CDownloadUIForIthacaDlg::OnOption() {
		//CString szPath;
		//::GetModuleFileName(NULL, szPath.GetBuffer(MAX_PATH), MAX_PATH);
		//szPath.ReleaseBuffer();
		//szPath = szPath.Left(szPath.ReverseFind(_T('\\'))+1);
	...
	void CDownloadUIForIthacaDlg::OnAboutBox() {
		CAboutBox AboutBox;
		AboutBox.DoModal();
	}

	bool CDownloadUIForIthacaDlg::GetDLPathFromIni(const TSTRING& strIniPath, const TSTRING& strName, TSTRING& strPath) {
		bool bRes = false;
		TCHAR szPath[500] ={0};
	...
	void CDownloadUIForIthacaDlg::InsertStringList(CString szMessage, CString szLabelInfo) {
	...
	bool CDownloadUIForIthacaDlg::SetIniPath() {
	...
	bool CDownloadUIForIthacaDlg::CheckSignedCode() {
	...
	bool CDownloadUIForIthacaDlg::ExtractSpecifiedString(const TSTRING& strFileName, char* szCheckCode, int nCheckLength, int nExtractLength, int nIndexToGet, char* nOutput, int nOutputLength) {
		CFile file;
	...
	int CDownloadUIForIthacaDlg::GetCodeSecurityVersion(const TSTRING& strFileName, char* szCheckCode, int nCheckLength) {
		char szOutput[10] = {0};
	...
	bool CDownloadUIForIthacaDlg::GetCodeVersion(const TSTRING& strFileName, char* szCheckCode, int nCheckLength, CString& strVer) {
		char szOutput[10] = {0};
	...
	bool CDownloadUIForIthacaDlg::GetCodeSigned(const TSTRING& strFileName, char* szCheckCode, int nCheckLength) {
		char szOutput[10] = {0};
	...
	bool CDownloadUIForIthacaDlg::CheckQFuseState1() {
	...
	bool CDownloadUIForIthacaDlg::CheckQFuseState2() {
		return false;
	}
	// Check QFuse after write NV 453 (FTM)
	bool CDownloadUIForIthacaDlg::CheckQFuseState3() {
	...
	bool CDownloadUIForIthacaDlg::GetVariantString(CString& strVariantString) {
		TSTRING strVar;
	...
	void CDownloadUIForIthacaDlg::ShowVariantVersion() {
	...
	void CDownloadUIForIthacaDlg::ShowSoftwareLoadVersion() {
	...
	void CDownloadUIForIthacaDlg::ShowSignedUnsignedCode() {
		TSTRING strAmss, strOemsbl;
		GetDLPathFromIni(m_strIni.GetString(), _T("amss"), strAmss);
		GetDLPathFromIni(m_strIni.GetString(), _T("oemsbl"), strOemsbl);
		if(strOemsbl.length() == 0) {
	...
	int CDownloadUIForIthacaDlg::ReadBatteryVoltage() {
	...
	void CDownloadUIForIthacaDlg::SetDownloadSettingParameters() {
		if(SetIniPath()) {
			TCHAR szBuf[20] ={0};
	...
	bool CDownloadUIForIthacaDlg::OpenImage(IMAGE & imgLoad, CString strLoadName) {
	...
	bool CDownloadUIForIthacaDlg::OpenAllImage() {
