...
#else
		g_hSysDll = ::LoadLibrary(_T("ODM_Export_StaticM_Melos.dll"));
#endif
	}
...
__VBDECLARATION  ODM_APIVersion( char * szApiVersion1024 , char *szErrMsg500 )
{
	...
	CString strVersion = "odmtpa_qisda_Melos_BTA.01.00.03";
	if(szApiVersion1024)
	...
	{	
		...
		if(bGetComPort && g_pIODMExport)
		{
			...
			if(nStatus!=TPTK_SUCCESS)
			{
				...
				nStatus = ODM_CONNECTCOMMPORT_ERROR << 8 | g_pIODMExport->GetLastError(nPort);
			}
		}
		else
...
__VBDECLARATION ODM_GenericWrite( long nPort, char*  szData1024, char*  szTag, char*  szErrMsg500 )
{
	...
	if(szTag && g_pIODMExport)
	{
		...
		else if(strcmp(szTag, "RUIM") == 0) // write simdata
		{
			...
			{
				int nRetry = 2;
				for(int i=0; i<nRetry && nStatus!=TPTK_SUCCESS; i++)
				{
					nStatus = g_pIODMExport->WriteSimData(nPort)? TPTK_SUCCESS : nStatus;
					if(nStatus!=TPTK_SUCCESS && (i+1)<nRetry)
						Sleep(5000);
				}
				if(nStatus != TPTK_SUCCESS)
				{
