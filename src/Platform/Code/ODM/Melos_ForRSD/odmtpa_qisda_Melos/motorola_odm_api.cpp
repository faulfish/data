...
bool GetComPort(long& nPort, char* szPortName)
{
	...
	if(szPortName)
	{
		...
		{		
			...
			//bGetComPort = D_GetMotoUSBPort_LONG(szPortName, "05c6","9004", nPort); //Melos Qualcomm pid/vid
			bGetComPort = D_GetMotoUSBPort_LONG(szPortName, "22B8","2D50", nPort); //Melos Moto pid/vid
		}
...
__VBDECLARATION  ODM_Initialize( char * szInitializeParameters, char*  szErrMsg500)
{
	...
	{
#ifdef _DEBUG
		g_hSysDll = ::LoadLibrary(_T("ODM_Export_StaticMD.dll"));
#else
		g_hSysDll = ::LoadLibrary(_T("ODM_Export_StaticM.dll"));
#endif
		if(g_hSysDll)
		{
			...
			if(D_NewODMExportInterface)
				nStatus = D_NewODMExportInterface(&g_pIODMExport) ? TPTK_SUCCESS : nStatus;
		}
	}
	else
...
{
	int nStatus = ODM_APIVERSION_ERROR << 8 | TPTK_FAIL;
	//initialize here
	if(!g_hSysDll)
	{
#ifdef _DEBUG
	...
	CString strVersion = "odmtpa_qisda_Melos_BTA.02.00.01";
	if(szApiVersion1024)
...
__VBDECLARATION  ODM_ConnectCommPort( long &nPort, char*   szPortName, char*  szErrMsg500 )
{
	...
	{	
		CString strPortName = szPortName;
		if(strPortName.Find("COM") == -1)
		{
			nPort = atoi(szPortName+4); //Get \USB"nPort"
			if(g_pIODMExport)
				nStatus = g_pIODMExport->ConnectComPort(nPort) ? TPTK_SUCCESS : nStatus;
			else
				_stprintf(szErrMsg500, "%d:ODM_ConnectCommPort Failed> Dll error", nPort);
		}
		else
			_stprintf(szErrMsg500, "%d:ODM_ConnectCommPort Failed> Not supoort RS232 Port", nPort);
	}
	else if(szErrMsg500)
...
__VBDECLARATION ODM_ReadIMEI( long nPort, char*   szIMEI100, char*  szErrMsg500 )
{
	...
	{	
		...
		{
			char szIMEI[100] = {0};
			nStatus = g_pIODMExport->ReadIMEI(nPort, szIMEI) ? TPTK_SUCCESS : ODM_READIMEI_ERROR;
			strcpy(szIMEI100, szIMEI);
		}
		if(nStatus != TPTK_SUCCESS)
		{
			if(szErrMsg500)
...
__VBDECLARATION ODM_GenericWrite( long nPort, char*  szData1024, char*  szTag, char*  szErrMsg500 )
{
	...
	if(szTag && g_pIODMExport)
	{
		...
		else if(strcmp(szTag, "RUIM") == 0) // write simdata
		{
